...
def scan_files(
    ...
    for file_info in files:
        ...
        if not content or not file_type:
            all_errors.append(
                {"file_path": file_path, "error": "Missing content or file type"}
            )
            continue
        ...
        try:
            ...
            all_errors.append(
                {"file_path": file_path, "error": f"Processing error: {str(e)}"}
            )
    ...
    return ScanResponse(findings=findings, summary=summary, errors=file_errors)
